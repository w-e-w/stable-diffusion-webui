        ...
        if not cache_path.exists():
            ...
            if (width > target_width > 0) or (height > target_height > 0):
                ...
                if getattr(img, 'n_frames', 1) > 1:
                    frames = ImageSequence.all_frames(img, lambda f: f.resize(new_size))
                    frames[0].save(cache_path, save_all=True, append_images=frames[1:], format=img.format)
                else:
                    img_resize = img.resize(new_size)
                    img_resize.save(str(cache_path), format=img.format)
                    save_image_with_geninfo(img_resize, geninfo, str(cache_path), image_format=img.format)
            else:
                return str(path_str)
